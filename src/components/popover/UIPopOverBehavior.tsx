...
import { UIPopOverTemplateProps } from "./UIPopOverTemplate";

interface UIPopOverBehaviorProps extends UIPopOverProps {
  Template: React.FC<UIPopOverTemplateProps>
}

const UIPopOverBehavior: React.FC<UIPopOverBehaviorProps> = (props) => {
  ...
  const currentOpenValue = ((): boolean => {
    if(props.id) {
      ...
      return (popover ? popover[1].open : false) as boolean
    }
  ...
  const getWidth = useCallback((): number => {
    if(typeof props.width === 'string') {
      switch (props.width) {
        case 'inherit':
          return popoverRef.current && popoverRef.current.parentElement ? popoverRef.current.parentElement.offsetWidth : 0
        case 'anchor':
...
export default UIPopOverBehavior