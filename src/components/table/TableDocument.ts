...
export class TableDocument<T> extends EventEmitter {
  private _data!: T[];
  private _loading = true;

  private readonly _columns: string[];
  private readonly _rows: Map<unknown, RowType<T>>;
  ...
  public getMaxPage(): number {
    const maxPage = Math.ceil(this._data.length / this.maxRows);
  ...
  private getPageRows(): T[] {
    return this._data.slice(this.maxRows * (this._pageNumber - 1), this.maxRows * this._pageNumber);
  }
}
