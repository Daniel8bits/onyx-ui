...
export default template<
  TextfieldTemplateProps, 
  HTMLInputElement, 
  TextfieldTemplateStyle
>({
  name: 'textfield',
  jsx: (props, data) => (
    <div 
    ...
    >
      {props.label
        && <label htmlFor={props.inputProps.id} className={data.theme?.div[1].label}>
          {props.label}
        </label>}
      <div ref={props.iconContainerRef} className={data.theme?.div[1].div[0]}>
        {props.icon && (!props.iconPosition || props.iconPosition === 'left')
          && <button 
            type='button' 
            onClick={props.disabled ? undefined : props.onClickIcon} 
            aria-label='button'
            className={data.theme?.div[1].div[1].button[0]}
          >
            <props.icon className={data.theme?.div[1].div[1].button[1].icon} />
          </button>}

        <input ref={props.el} className={data.theme?.div[1].div[1].input} {...props.inputProps} />
