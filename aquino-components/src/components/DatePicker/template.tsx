...
import type DatePickerCore from './common/DatePickerCore';
...
export default template<DatePickerTemplateProps, HTMLDivElement, DatePickerTemplateStyle>({
  name: 'datepicker',
  jsx(props, data) {
    const popOverSize = {
      width: 270,
    ...
    return (
      <div ref={el} className={`${data.theme?.div[0] ?? ''} ${props.className ?? ''}`} {...events}>
        <PopOver
          id={popoverId}
          width={popOverSize.width}
          height={popOverSize.height}
          anchor={iconButtonRef}
          className={data.theme?.div[1].popover}
        >
          {currentPanel}
        </PopOver>
        <MaskedTextfield {...inputTextProps}
          ...
          className={data.theme?.div[1].textfield}
        />
      </div>
    );
  }, 
